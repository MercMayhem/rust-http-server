...
use html::{text_content::UnorderedList , content::Header, root::{Body, Html}, metadata::Head};
...
fn get_head(dir_resource: &PathBuf) -> Head{
    let dir_name = dir_resource.file_name()
                          .map(|name| name.to_string_lossy().into_owned())
                          .map(|name| {
                            if env::current_dir().ok().is_some_and(|path| path == *dir_resource){
                                return "/".into(); 
                            } else {
                                return name;
                            }
...
fn get_dir_as_heading(dir_resource: &PathBuf) -> Header{
    return Header::builder()
        .heading_1(|h1| h1.text(dir_resource
                    .file_name()
                    .map(|name| name.to_string_lossy().into_owned())
                    .map(|mut name| {
                        if env::current_dir().ok().is_some_and(|path| path == *dir_resource){
...
fn get_files_as_ul(dir_resource: &PathBuf) -> UnorderedList{
    ...
    for file in all_files{
        ...
        if file.is_dir(){
            ...
            list_builder.list_item(|li| li.anchor(|a| {
